...
export function runMigrations(db: Database.Database, options: MigrationOptions = {}) {
  ...
  if (userVersion < 4) {
    ...
    if (!hasColumn("tool_runs", "actor_id")) {
      db.exec(`ALTER TABLE tool_runs ADD COLUMN actor_id TEXT;`);
    }
    if (!hasColumn("kb_docs", "owner")) {
      db.exec(`ALTER TABLE kb_docs ADD COLUMN owner TEXT;`);
    }
    if (!hasColumn("kb_docs", "visibility")) {
      db.exec(`ALTER TABLE kb_docs ADD COLUMN visibility TEXT;`);
    }
