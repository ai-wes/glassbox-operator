...
Usage:
    ...
    python examples/asr_client.py audio.wav --url ws://localhost:8099/ws/asr
...
async def transcribe_audio(
    audio_path: str,
    server_url: str = "ws://localhost:8099/ws/asr",
    realtime: bool = False,
    chunk_ms: int = 100,
    ...
    async with websockets.connect(server_url) as ws:
        ...
        if session_id:
            await ws.send(json.dumps({"type": "start", "sessionId": session_id}))
...
def main():
    ...
    parser.add_argument(
        "--url",
        default="ws://localhost:8099/ws/asr",
        help="WebSocket server URL (default: ws://localhost:8099/ws/asr)",
    )
    parser.add_argument(
        "--realtime",
        action="store_true",
    ...
    parser.add_argument(
        "--session-id",
        help="OpenCode session id to forward transcript",
    )
