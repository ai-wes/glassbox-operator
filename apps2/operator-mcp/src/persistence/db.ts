...
export class OperatorDb {
  ...
  // Tool runs (audit)
  // -----------------------
  startToolRun(row: { id: string; toolName: string; serverName: string; startedAt: string; argsJson: string | null }) {
    const stmt = this.db.prepare(`
      INSERT INTO tool_runs (id, tool_name, server_name, started_at, arguments_json)
      VALUES (?, ?, ?, ?, ?)
    `);
    stmt.run(row.id, row.toolName, row.serverName, row.startedAt, row.argsJson);
  }
  ...
  upsertKbDoc(doc: KbDocRow) {
    const stmt = this.db.prepare(`
      INSERT INTO kb_docs (id, type, title, body, source, tags_json, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    ...
    `);
    stmt.run(doc.id, doc.type, doc.title, doc.body, doc.source, doc.tags_json, doc.created_at, doc.updated_at);
  }
  ...
  listKbDocs(limit: number, offset: number): Array<Pick<KbDocRow, "id" | "type" | "title" | "source" | "tags_json" | "created_at" | "updated_at">> {
    const stmt = this.db.prepare(`
      SELECT id, type, title, source, tags_json, created_at, updated_at
      FROM kb_docs
  ...
  searchKb(query: string, limit: number) {
    ...
    try {
      ...
      const stmt = this.db.prepare(`
        SELECT
          id, type, title, source, tags_json, updated_at,
  ...
  listDrafts(limit: number, offset: number, kind?: string, status?: string): DraftRow[] {
  ...
  // -----------------------
  linkDoc(row: { id: string; fromKind: "tool_run" | "draft"; fromId: string; toDocId: string; relation: string; createdAt: string }) {
    const stmt = this.db.prepare(`
