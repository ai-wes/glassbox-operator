...
function buildManualSpecForKnownTypes(actionType: string, draft?: DraftRow | null): ExecutionSpec | null {
  ...
  if (lowerType === "publish_post") {
    if (draft && draft.kind === "linkedin_post") {
      return {
        kind: "manual",
        instruction:
          "LinkedIn has no API automation in this stack. Draft marked ready_to_post. Copy/paste into LinkedIn and publish.",
        draftStatusUpdates: [{ draftId: draft.id, status: "ready_to_post" }]
      };
    }
    if (draft && draft.kind === "blog_post") {
      return {
        kind: "manual",
        instruction:
          "Blog publishing not wired yet. Draft marked ready_to_publish. Wire an executor to Glassbox MCP when ready.",
        draftStatusUpdates: [{ draftId: draft.id, status: "ready_to_publish" }]
      };
    }
    return {
      kind: "unsupported",
      reason: "publish_post requires a draft payload_ref and a known draft kind (linkedin_post/blog_post)."
    };
  }

  if (lowerType === "send_email") {
    if (draft && draft.kind === "email") {
      return {
        kind: "manual",
        instruction:
          "Email send is disabled by default in execution v0. Draft marked ready_to_send. You can wire a Gmail executor to create a Gmail draft (safe) or explicitly allow sending.",
        draftStatusUpdates: [{ draftId: draft.id, status: "ready_to_send" }]
      };
    }
    return { kind: "unsupported", reason: "send_email requires an email draft payload_ref." };
  }
      ...
      planned.push({
        ...
        spec: {
          ...
          safety: {
            ...
            wouldSendEmail: (ex.operation || "").toLowerCase() === "send" && a.action_type === "send_email"
          }
        }
      });
      continue;
